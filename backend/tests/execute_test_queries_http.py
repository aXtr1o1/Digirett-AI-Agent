#!/usr/bin/env python3
"""
execute_test_queries_http.py

SIMPLER HTTP VERSION - Runs real queries against backend using HTTP instead of WebSocket.

This script:
1. Uploads test documents via API (multipart/form-data)
2. Executes all 20 test queries via HTTP POST
3. Captures real LLM-generated answers
4. Displays results in terminal with color coding
5. Saves results to JSON file

Usage:
    python execute_test_queries_http.py
...
Advantages over WebSocket version:
    - Simpler HTTP protocol (more reliable)
    - No async complexity issues
...
class SimpleQueryExecutor:
    """Execute test queries against backend using simple HTTP calls."""
    ...
    def __init__(self, backend_url: str = "http://localhost:8000"):
        ...
        self.uploaded_docs = []        # Create event loop once for all async operations
        try:
    ...
    def _load_scenarios(self) -> Dict[str, Any]:
        ...
        if not scenarios_file.exists():
            print(f"{Colors.RED}❌ Test scenarios file not found: {scenarios_file}{Colors.ENDC}")
            sys.exit(1)
    ...
    def run_test_scenarios(self) -> None:
        """Execute all test scenarios and queries."""
        self.print_header("DIGIRETT DOCUMENT FEATURE TEST - LIVE EXECUTION")
        ...
        # Execute scenarios
        self.print_section("Step 2: Executing Test Queries")
        scenario_results = []
        ...
        for scenario_idx, scenario in enumerate(self.test_scenarios.get("test_scenarios", []), 1):
            ...
            for query_idx, query_def in enumerate(scenario.get("queries", []), 1):
                ...
                if response.get("error"):
                    ...
                    print(f"  Intent: {Colors.CYAN}{intent}{Colors.ENDC}")
                    print(f"  Score: {Colors.GREEN}{score:.2f}{Colors.ENDC} | Tokens: {Colors.BLUE}{tokens}{Colors.ENDC}")
    ...
    def save_results(self) -> None:
        ...
        try:
            ...
            print(f"\n{Colors.GREEN}✅ All done! Check test_results.json for details.{Colors.ENDC}\n")
