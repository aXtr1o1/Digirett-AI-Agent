...
export default function LawyerDashboard() {
  ...
  const fetchData = async (isManual = false) => {
    ...
    try {
      const [queueData, myData] = await Promise.all([
        ...
        hitlService.getMyAssignedTickets()
      ]);
  ...
  const handleClaim = async (ticketId) => {
    try {
      ...
      setQueueMsg({ type: "success", text: "Ticket claimed successfully! It's now in your Intake." });
      fetchData();
    } catch (err) {
  ...
  const intakeTicket = myTickets.length > 0 ? myTickets[0] : null;
  const pendingTickets = myTickets.length > 1 ? myTickets.slice(1) : [];

  // Dynamic Data Calculations - No more fake values
  const allTickets = [...queueTickets, ...myTickets];

  const allocationData = [
    { name: 'Available', value: queueTickets.length, color: '#6366f1' },
    { name: 'My Intake', value: myTickets.length, color: '#a855f7' },
  ...
  const stats = [
    { label: "My Cases", value: myTickets.length, icon: Briefcase, color: "blue" },
    ...
    { label: "Resolved", value: "0", icon: CheckCircle2, color: "emerald" }
  ];
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${isDark ? "bg-[#020617] text-slate-200" : "bg-[#f1f5f9] text-slate-900"}`}>
      ...
      {/* SIDEBAR */}
      <aside className={`fixed lg:relative z-50 inset-y-0 left-0 w-64 transform transition-transform duration-300 ease-in-out border-r ${
        isDark ? "bg-slate-900 border-slate-800" : "bg-[#0f172a] border-slate-800"
              ...
              <nav className="space-y-1">
                <button 
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-bold transition-all ${
                    activeView === "dashboard" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-white hover:bg-white/5"
                  }`}
                >
                ...
                <button 
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-bold transition-all ${
                    activeView === "queue" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-white hover:bg-white/5"
                  }`}
                >
            ...
            <div>
              ...
              <nav className="space-y-1">
                <button 
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-bold transition-all ${
                    activeView === "intake" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-white hover:bg-white/5"
                  }`}
                >
                  <Inbox size={16} />
                  Intake Case
                  {intakeTicket && <span className="ml-auto bg-indigo-500 w-2 h-2 rounded-full animate-pulse" />}
                </button>
                <button 
                  onClick={() => { setActiveView("pending"); setIsSidebarOpen(false); }}
                  className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-bold transition-all ${
                    activeView === "pending" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-white hover:bg-white/5"
                  }`}
                >
                  <Layers size={16} />
                  Pending Matters
                  <span className="ml-auto text-slate-500 text-[10px]">{pendingTickets.length}</span>
                </button>
                <button 
                  onClick={() => { setActiveView("notes"); setIsSidebarOpen(false); }}
                  className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-bold transition-all ${
                    activeView === "notes" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-white hover:bg-white/5"
                  }`}
                >
                  <Bookmark size={16} />
                  Notes
            ...
            <div className="relative">
              ...
              >
                <div className={`h-8 w-8 rounded-full flex items-center justify-center font-bold text-xs transition-transform group-hover:scale-105 ${
                  isDark ? "bg-indigo-500/20 text-indigo-400" : "bg-indigo-100 text-indigo-700 shadow-sm"
                }`}>
              ...
              {showProfileDropdown && (
                <div className={`absolute top-full right-0 mt-2 w-52 rounded-xl border shadow-2xl z-[100] overflow-hidden animate-in fade-in zoom-in-95 duration-200 ${
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className={`border rounded-2xl shadow-sm overflow-hidden ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
                <div className={`px-8 py-5 border-b ${isDark ? "border-slate-800" : "border-slate-100"}`}>
                  <h2 className="text-base font-bold">Global Assistance Queue</h2>
                  <p className="text-xs text-slate-500 mt-1">Shared pool of open legal escalations waiting for assignment.</p>
                </div>

                {queueMsg && (
                  <div className={`mx-8 mt-6 p-4 rounded-xl flex items-center justify-between text-sm font-bold animate-in fade-in zoom-in-95 ${
                    queueMsg.type === 'success' 
                     ...
                     <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {pendingTickets.map((t) => (
                          <tr key={t.ticket_id} className="hover:bg-slate-50/50 transition-colors">
                             ...
                             <td className="px-8 py-6 text-xs font-medium text-slate-500">{formatDate(t.assigned_at)}</td>
                             <td className="px-8 py-6 text-right">
                                <button onClick={() => navigate(`/lawyer/tickets/${t.ticket_id}`)} className="text-indigo-500 text-xs font-bold hover:underline">View Case</button>
                             </td>
                          </tr>
                        ))}
                     </tbody>
