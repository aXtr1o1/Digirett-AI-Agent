...
@patch("boto3.client")
def test_embedder_init(mock_client):
    e = SageMakerBGEEmbedder("ep")
    assert e.endpoint_name == "ep"

@patch("boto3.client")
def test_embed_empty_chunks(mock_client):
    e = SageMakerBGEEmbedder("ep")
    assert e.embed_chunks([]) == []

@patch("boto3.client")
def test_embed_invalid_text(mock_client):
    e = SageMakerBGEEmbedder("ep")
...
@patch("boto3.client")
def test_invoke_batch_failure(mock_client):
    e = SageMakerBGEEmbedder("ep")
    e.client.invoke_endpoint.side_effect = Exception("fail")
    assert e._invoke_batch(["x"]) == [None]

@patch("boto3.client")
def test_embed_assigns_embedding(mock_client):
    e = SageMakerBGEEmbedder("ep")
