...
def test_milvus_init(_, __, ___):
    store = MilvusTextStore("h", 19530, "c", 3)
    assert store.collection_name == "c"
...
def test_close_disconnect():
    with patch("ingestion.src.storage.milvus_store.connections.disconnect") as d:
        MilvusTextStore.close(MilvusTextStore.__new__(MilvusTextStore))
        d.assert_called()
def test_fix_embedding_dimension_mismatch():
    s = MilvusTextStore.__new__(MilvusTextStore)
    s.embedding_dim = 5  # expected 5
    emb = [1, 2, 3]      # only 3 provided

    result = s._fix_embedding(emb)

    # Should still return floats even if dimension mismatches
